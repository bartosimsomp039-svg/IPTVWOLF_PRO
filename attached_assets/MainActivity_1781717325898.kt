...
// 🔥 1. CACHE EN MEMORIA
        ...
        lifecycleScope.launch {
            var cacheJson: String? = withContext(Dispatchers.IO) {
                // Busca primero la clave account-indexed ("0_live")
                // Si no existe, usa la clave de HomeActivity como fallback
                CacheManager.obtener(this@MainActivity, DataRepository.currentUser, accountCacheKey)
                    ?: if (isM3U)
                        CacheManager.obtener(this@MainActivity, DataRepository.currentUser, "m3u_$type")
                    else
                        CacheManager.obtener(this@MainActivity, DataRepository.currentUser, type)
            }

            // ── Solo para M3U series: verificar cache de episodios ──
            if (isM3U && type == "series") {
                val hasEpCache = withContext(Dispatchers.IO) {
                    !CacheManager.obtener(
                        this@MainActivity,
                ...
                if (isM3U && type == "series" && DataRepository.parsedSeriesCache.isEmpty()) {
                    launch(Dispatchers.IO) {
                        try {
                            ...
                            val fullJson = CacheManager.obtener(
                                this@MainActivity,
                                DataRepository.currentUser,
